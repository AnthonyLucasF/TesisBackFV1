...
export const postLiquidacion = async (req, res) => {
  try {
    let { lote_id, tipo } = req.body;
    if (!lote_id || !tipo) {
      return res.status(400).json({ message: "lote_id y tipo requeridos" });
    }

    // Normalizar tipo enviado desde frontend a la descripción de la DB
    let tipoDB;
    ...
    // Emitir evento WebSocket si aplica
