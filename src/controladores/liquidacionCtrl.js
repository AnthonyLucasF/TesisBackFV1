    ...
    const [cab] = await conmysql.query(`
      SELECT 
        ...
        CASE 
          ...
          THEN (li.total_libras / lo.lote_libras_remitidas) 
          ELSE 0 
...
export const postLiquidacion = async (req, res) => {
  try {
    ...
    // Rendimiento en base a libras remitidas
    const rendimiento =
      librasRemitidas > 0
    ...
    const [liq] = await conmysql.query(`
      INSERT INTO liquidacion 
      (lote_id, liquidacion_tipo, liquidacion_rendimiento, liquidacion_basura, liquidacion_sobrante, total_libras)
      VALUES (?, ?, ?, ?, ?, ?)
    `, [lote_id, tipoBD, rendimiento, totalBasura, sobrante, totalLibras]);
